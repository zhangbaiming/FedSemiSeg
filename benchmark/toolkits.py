...
class DefaultTaskGen(BasicTaskGen):
    def __init__(self, benchmark, dist_id, skewness, rawdata_path, num_clients=1, minvol=10, local_hld_rate=0.2, seed=0):
        super(DefaultTaskGen, self).__init__(benchmark, dist_id,
                                             skewness, rawdata_path, local_hld_rate, seed)
        self.minvol = minvol
        self.num_classes = -1
    ...
    def partition(self):
        ...
        if self.dist_id == 0:
            ...
            for lb in lb_counter.keys():
                ...
                local_datas = [local_data+lb_data.tolist()
            ...
            samples_per_client = np.random.lognormal(
                mu, sigma, (self.num_clients)).astype(int)
            ...
            while crt_data_size != total_data_size:
                if crt_data_size - total_data_size >= thresold:
                    ...
                    maxid = np.argmax(samples_per_client)
                    samples_per_client[maxid] -= (crt_data_size -
                                                  total_data_size)
                    ...
                    minid = np.argmin(samples_per_client)
                    samples_per_client[minid] += (
                        total_data_size - crt_data_size)
                crt_data_size = sum(samples_per_client)
            # count the label distribution
            labels = [self.train_data[did][-1]
                      for did in range(len(self.train_data))]
            lb_counter = collections.Counter(labels)
            p = np.array([1.0 * v / len(self.train_data)
                         for v in lb_counter.values()])
            lb_dict = {}
            labels = np.array(labels)
            for lb in range(len(lb_counter.keys())):
                lb_dict[lb] = np.where(labels == lb)[0]
            proportions = [np.random.dirichlet(
                alpha * p) for _ in range(self.num_clients)]
            while np.any(np.isnan(proportions)):
                proportions = [np.random.dirichlet(
                    alpha * p) for _ in range(self.num_clients)]
            sorted_cid_map = {k: i for k, i in zip(np.argsort(samples_per_client), [
                                                   _ for _ in range(self.num_clients)])}
            crt_id = 0
            while True:
                # generate dirichlet distribution till ||E(proportion) - P(D)||<=1e-5*self.num_classes
                mean_prop = np.sum(
                ...
                for cid in range(self.num_clients):
                    if np.any(np.isnan(sup_prop[cid])):
                        continue
                    alter_prop = del_prop + \
                        samples_per_client[excid]*sup_prop[cid]
            ...
            for lb in lb_counter.keys():
                ...
                lb_proportion = lb_proportion / lb_proportion.sum()
                lb_proportion = (np.cumsum(lb_proportion) *
                                 len(lb_idxs)).astype(int)[:-1]
                lb_datas = np.split(lb_idxs, lb_proportion)
                self.dirichlet_dist.append(
                    [len(lb_data) for lb_data in lb_datas])
                local_datas = [local_data + lb_data.tolist()
                               for local_data, lb_data in zip(local_datas, lb_datas)]
            self.dirichlet_dist = np.array(self.dirichlet_dist).T
            for i in range(self.num_clients):
    ...
    def visualize_by_class(self, train_cidxs):
        ...
        if hasattr(self, 'dirichlet_dist'):
            ...
            row_map = {k: i for k, i in zip(np.argsort(data_columns), [
                                            _ for _ in range(self.num_clients)])}
            for cid, cprop in enumerate(client_dist):
                ...
                for lbi in range(len(cprop)):
                    plt.fill_between([offset, offset+cprop[lbi]],
                                     y_bottom, y_top, facecolor=colors[lbi])
            ...
            for cid, cidxs in enumerate(train_cidxs):
                ...
                for lbi in range(self.num_classes):
                    plt.fill_between(
                        [offset, offset+lb_counter[lbi]], y_bottom, y_top, facecolor=colors[lbi])
...
class BasicTaskCalculator:
    ...
    def get_optimizer(self, model=None, lr=0.1, weight_decay=0, momentum=0):
        ...
        OPTIM = getattr(importlib.import_module(
...
class ClassificationCalculator(BasicTaskCalculator):
    ...
    def test(self, model, dataset, batch_size=64, num_workers=0):
        ...
        for batch_id, batch_data in enumerate(data_loader):
            ...
            correct = y_pred.eq(
                batch_data[-1].data.view_as(y_pred)).long().cpu().sum()
...
class XYTaskPipe(BasicTaskPipe):
    ...
    def load_task(cls, task_path):
        ...
        test_data = cls.TaskDataset(
            feddata['dtest']['x'], feddata['dtest']['y'])
        train_datas = []
        valid_datas = []
