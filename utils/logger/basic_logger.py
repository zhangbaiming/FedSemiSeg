...
class Logger(logging.Logger):
    ...
    def __init__(self, meta, *args, **kwargs):
        ...
        if not self.meta['no_log_console']:
            ...
            self.streamhandler.setLevel(
                self._LEVEL[self.meta['log_level'].upper()])
    ...
    def save_output_as_json(self, filepath=None):
        ...
        if filepath is None:
            filepath = os.path.join(
                self.get_output_path(), self.get_output_name())
        ...
        for key, value in self.output.items():
            if not self.check_is_jsonable(value):
                try:
                    ...
                    self.warning(
                        "flw.logger.output['{}'] is not jsonable, and is automatically converted to string.".format(key))
                except:
                    del self.output[key]
                    self.warning(
                        "Automatically remove flw.logger.output['{}'] from logger, because it is not jsonable and is failed to convert into string. ".format(key))
        return
    ...
    def get_output_name(self, suffix='.json'):
        ...
        if self.meta['num_steps'] < 0:
            output_name = output_name + \
                ("E{}_".format(self.meta['num_epochs']))
        else:
    ...
    def early_stop(self):
        ...
        if self._es_best_score is None:
            ...
            if self._es_counter >= self._es_patience:
                self.info('Early stopping after training for {} rounds.'.format(
