...
sample_list = ['uniform', 'md', 'full', 'labeled']
agg_list = ['uniform', 'weighted_scale', 'weighted_com', 'weighted_entropy', 'weighted_entropy_dis', 'weighted_consistency']
optimizer_list = ['SGD', 'Adam', 'RMSprop', 'Adagrad']
agg_label_list = ['proto_sim', 'proto_sim_max', 'proto_dis', 'proto_dis_max', 'outs_sim', 'outs_sim_max',
                  'outs_dis', 'outs_dis_max', 'model_sim', 'model_sim_max', 'model_dis', 'model_dis_max', 'random', 'weighted_scale', 'none']
logger = None
...
def read_option():
    ...
    parser.add_argument('--test_batch_size', help='the batch_size used in testing phase;', type=int, default=1)
    ...
    parser.add_argument('--weight_contrastive', help='weight of contrastive loss', type=float, default=1)
    parser.add_argument('--num_clusters', help='number of prototypical clusters', type=int, default=-1)
    parser.add_argument('--labeledclients', nargs='*', help='Labeled Clients IDs', type=int, default=[0, 1, 2])
    parser.add_argument('--numlabeled', help='the number of Labeled Clients', type=int, default=5)
    parser.add_argument('--aggregate_labeled', help='methods for aggregating labeled models', type=str, choices=agg_label_list, default='proto_sim')
    parser.add_argument('--agg_coefficient', help='coefficient for aggregating global model', type=float, default=0.5)
    parser.add_argument('--num_normimg', help='number of random Gaussian images', type=int, default=1)
    parser.add_argument('--contrastive', help='bool controls whether to use contrastive', action="store_true", default=False)
    parser.add_argument('--size_bank', help='size of prototypical bank', type=int, default=5)
    parser.add_argument('--ema', help='coefficient of ema', type=float, default=0.9)
    parser.add_argument('--vis', help='bool controls whether to visualize results', action="store_true", default=False)

    try:
        option = vars(parser.parse_args())
        print(option['contrastive'])
...
def initialize(option):
    ...
    logger_order = {'{}Logger'.format(option['algorithm']): '%s.%s' % ('algorithm', option['algorithm']), option['logger']: '.'.join(
        ['utils', 'logger', option['logger']]), 'basic_logger': '.'.join(['utils', 'logger', 'basic_logger'])}
    global logger
    ...
    num_clients = int(option['task'][option['task'].find('cnum')+4:option['task'].find('dist')-1])

    if len(option['labeledclients']) == 0:
        labeledclients = random.sample(range(num_clients), option['numlabeled'])
    else:
        labeledclients = option['labeledclients']

    print(labeledclients)

    if 'semi' in option['algorithm']:
        train_datas, valid_datas, test_data, client_names = TaskPipe.load_task(os.path.join('fedtask', option['task']), labeledclients)
    else:
        train_datas, valid_datas, test_data, client_names = TaskPipe.load_task(os.path.join('fedtask', option['task']))

    if 'seg' in option['logger']:
        logger = Logger(meta=option, labeledclients=labeledclients, name=log_name, level=option['log_level'])
    else:
    ...
    try:
        utils.fmodule.SvrModel = getattr(
            importlib.import_module(bmk_model_path), 'SvrModel')
    ...
    try:
        if option['pretrain'] != '':
            ...
            logger.info('The pretrained model parameters in {} will be loaded'.format(
                option['pretrain']))
    except:
    ...
    if 'semi' in option['algorithm']:
        client_states = [True if c in labeledclients else False for c in range(num_clients)]
        clients = [Client(option, name=client_names[cid], train_data=train_datas[cid], valid_data=valid_datas[cid], islabeled=client_states[cid]) for cid in range(num_clients)]
    else:
    ...
    logger.info('Initializing systemic heterogeneity: ' +
                'Network Environment: {} \\ Computing Power Allocation: {}'.format(option['network_config'], option['computing_config']))
...
def output_filename(option, server):
    ...
    for para, pv in server.algo_para.items():
