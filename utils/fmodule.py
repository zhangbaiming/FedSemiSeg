...
class FModule(nn.Module):
    ...
    def __add__(self, other):
        if isinstance(other, int) and other == 0:
            return self
        if not isinstance(other, FModule):
            raise TypeError
    ...
    def count_parameters(self, output=True):
        try:
            ...
            print(
                'Please install prettytable through `pip install prettytable` before calling this func')
            return
...
def element_wise_func(m, func):
    ...
    if m.ingraph:
        ...
        _modeldict_cp(res.state_dict(),
                      _modeldict_element_wise(m.state_dict(), func))
    return res
    ...
    if op_with_graph:
        ...
        for n in range(len(mlr)):
            ...
            for l in mlr[n]._parameters.keys():
                if mlr[n]._parameters[l] is None:
                    continue
...
def _model_average(ms=[], p=[]):
    if len(ms) == 0:
        return None
    if len(p) == 0:
        p = [1.0 / len(ms) for _ in range(len(ms))]
    ...
    if op_with_graph:
        ...
        _modeldict_cp(res.state_dict(), _modeldict_weighted_average(
            [mi.state_dict() for mi in ms], p))
    return res
    ...
    if op_with_graph:
        ...
        for n1, n2, nr in zip(ml1, ml2, mlr):
            ...
            for l in nr._parameters.keys():
                if nr._parameters[l] is None:
                    continue
                nr._parameters[l] = rd[l]
    else:
        _modeldict_cp(res.state_dict(), _modeldict_add(
...
def _model_sub(m1, m2):
    ...
    if op_with_graph:
        ...
        _modeldict_cp(res.state_dict(), _modeldict_sub(
            m1.state_dict(), m2.state_dict()))
    return res
    ...
    if op_with_graph:
        ...
        for n in ml:
            for l in n._parameters.keys():
                if n._parameters[l] is None:
                    continue
                if n._parameters[l].dtype not in [torch.float, torch.float32, torch.float64]:
                    continue
...
def _modeldict_weighted_average(mds, weights=[]):
    ...
    if len(weights) == 0:
        weights = [1.0 / len(mds) for _ in range(len(mds))]
    for wid in range(len(mds)):
...
def _modeldict_cossim(md1, md2):
    ...
    for layer in md1.keys():
        if md1[layer] is None or ('weight' not in layer and 'bias' not in layer):
            continue
        res += (md1[layer].view(-1).dot(md2[layer].view(-1)))
...
def with_multi_gpus(func):
    def cal_on_personal_gpu(self, model, *args, **kargs):
        ...
        new_kargs = {}
        for arg in args:
            narg = arg.to(self.device) if hasattr(
                arg, 'get_device') or hasattr(arg, 'device') else arg
            new_args.append(narg)
        for k, v in kargs.items():
            nv = v.to(self.device) if hasattr(
                v, 'get_device') or hasattr(v, 'device') else v
            new_kargs[k] = nv
        ...
        if res is not None:
            if type(res) == dict:
                for k, v in res.items():
                    nv = v.to(origin_device) if hasattr(
                        v, 'get_device') or hasattr(v, 'device') else v
                    res[k] = nv
                ...
                res = res.to(origin_device) if hasattr(
                    res, 'get_device') or hasattr(res, 'device') else res
        return res
